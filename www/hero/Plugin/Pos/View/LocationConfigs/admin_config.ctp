...
<script type="text/javascript">
        ...
        //	Handle delete event
        $('.js-delete-config').click(function(){
        if(isFormLoading())
        ...
        location.href='<?php echo $this->Html->url('/admin'.$langPath.'pos/location_configs/delete/'); ?>'+configId+'<?php echo $backUrlParamsStr; ?>';
        return false;
        });
...
<?php
        ...
        'title'=>__d('pos','config_by_location'),
        'color'=>'grey',
        'minHeight'=>'580px',
        )
        );
        ...
        array('name'=>__d('pos',$variableKeyMapping[$variable]),'link'=>'')
        );

        echo $this->element('breadcrumb',array('lists'=>$breadCrumbLists));

        //////////////////////////////////////////////////////////////////////////////
        //	Show add new button and search bar
        echo'<div style="width:100%;padding-top:5px;">';
        echo'<div style="float:left;">'
        .$this->element('button',array('id'=>'NewButton','content'=>__('add_new'),'link'=>$this->Html->url('/admin'.$langPath.'pos/location_configs/add/'.$variable.$backUrlParamsStr)))
        .'</div>';
        echo'</div>';
        ...
        //////////////////////////////////////////////////////////////////////////////
        echo'<table class="list-basic list-border header-align-center" width="99%">';

        if(!empty($systemConfigsByLocations)){

        //	Header line
        echo'<tr><th colspan=3 width="2%"></th>'
        .'<th width>'.__('name').'</th>'
        .'<th width>'.__('value').'</th>'
        .'</tr>';
        ...
        switch($location){
        case'':
        $level=__d('pos','all_locations');
        break;
        case'shop':
        $level=__d('outlet','shop');
        break;
        case'outlet':
        $level=__d('outlet','outlet');
        break;
        case'station':
        $level=__d('pos','station');
        break;
default:
        $level='&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;';
        break;
        ...
        echo'<tr>'
        .'<td colspan="5">'
        .'<span class="sub-title-name">'.$level.'</span>'
        .'</td>'
        .'</tr>';

        //	Show system configs under the location
        foreach($systemConfigs as $systemConfig){

        $rowClass='';
        if(ENTERPRISE_CONFIG){
        if($location==''&&empty($bIsSupremeConfigZone))
        $rowClass=' class="ancestor"';
        }
        echo'<tr'.$rowClass.'>';

        echo'<td>'
        .$this->Html->link(
        $this->Html->image('icons/16x16/zoom.png',array('alt'=>__('view_detail'),'title'=>__('view_detail'))),
        '/admin'.$langPath."pos/location_configs/view/".$systemConfig['scfg_id'].$backUrlParamsStr,
        array('escape'=>false)
        )
        .'</td>';
        echo'<td>'
        .$this->Html->link(
        $this->Html->image('icons/16x16/pencil.png',array('alt'=>__('edit'),'title'=>__('edit'))),
        '/admin'.$langPath."pos/location_configs/edit/".$systemConfig['scfg_id'].$backUrlParamsStr,
        array('escape'=>false)
        )
        .'</td>';
        echo'<td>'
        .$this->Html->link(
        $this->Html->image('icons/16x16/trash.png',array('alt'=>__('delete'),'title'=>__('delete'))),
        'javascript:;',
        array('class'=>'js-delete-config','configId'=>$systemConfig['scfg_id'],'escape'=>false)
        )
        .'</td>';
        ...
        switch($location){
        case'':
        $curLocation=__d('pos','all_locations');
        break;
        case'outlet':
        case'shop':
        $code=key($systemConfig['scfg_record']);
        $name=current($systemConfig['scfg_record']);
        $curLocation=$name.(empty($code)?'':' ('.$code.')');
        break;
        case'station':
        $curLocation=$systemConfig['scfg_record'];
        break;
default:
        $curLocation='&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;';
        break;
        }
        echo'<td>'.$curLocation.'</td>';
        ...
        $value=$systemConfig['scfg_value'];
        break;
        case'menu_mode':
        case'show_page_up_and_down_button_for_list':
        $value=$systemConfig['scfg_value'].' ('.__d('pos','default_table_number_for_menu_mode').')';
        break;
        case'auto_switch_from_pay_result_to_starting_page_time_control':
        case'ordering_timeout':
        $value=$systemConfig['scfg_value'].' ('.__('seconds').')';
        break;
        case'time_control_to_open_next_check_by_member':
        $value=$systemConfig['scfg_value'].' ('.__('minutes').')';
        break;
        case'copies_of_receipt':
        $value=$systemConfig['scfg_value'].' ('.__('copies').')';
        break;
        case'payment_check_types':
        $mappings=json_decode($systemConfig['scfg_value'],true);
        ...
        $value=isset($yesNoOpts[$systemConfig['scfg_value']])?$yesNoOpts[$systemConfig['scfg_value']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;';
        break;
        ...
        for($i=0;$i<count($mappings);$i++){
        $value.='<tr>'
        .'<td width="45%">'.(isset($posFunctions[$mappings[$i]['function_key']])?$posFunctions[$mappings[$i]['function_key']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;').'</td>'
        .'<td width="45%">'.(isset($failHandlingOptions[$mappings[$i]['fail_handling']])?$failHandlingOptions[$mappings[$i]['fail_handling']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;').'</td>'
        .'<td width="10%">'.$mappings[$i]['seq'].'</td>'
        .'</tr>';
        }
        $value.='</table>';
        break;
        ...
        $values=json_decode($systemConfig['scfg_value'],true);
        ...
        .'</td>'
        .'<td>'
        .(isset($values['support_partial_payment'])&&isset($yesNoOpts[$values['support_partial_payment']])?$yesNoOpts[$values['support_partial_payment']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;')
        .'</td>'
        .'</tr>'
        .'<tr>'
        .'<td width="50%">'
        ...
        $value=isset($selectOpts[$systemConfig['scfg_value']])?$selectOpts[$systemConfig['scfg_value']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;';
        break;
        case'call_number_input_setting':
        $temp=array(
        'method'=>$systemConfig['scfg_value'],
        'image_file_for_scan_mode'=>'',
        );

        $values=is_array(json_decode($systemConfig['scfg_value'],true))?json_decode($systemConfig['scfg_value'],true):$temp;
        ...
        //	Payment methods
        $availablePaymentIds=explode(",",isset($values['available_payment_ids'])?$values['available_payment_ids']:"");
        ...
        .'</td>'
        .'<td>';
        foreach($availablePaymentIds as $availablePaymentId){
        if(isset($posPaymentMethods[$availablePaymentId]))
        $value.=__k($posPaymentMethods[$availablePaymentId],30).' ('.(in_array($availablePaymentId,$reusablePaymentIds)?__d('pos','reuse'):__d('pos','not_reuse')).')'.'<br />';
        }
        ...
        $values=json_decode($systemConfig['scfg_value'],true);
        $value='<table class="no-border" width="100%">'
        .'<tr>'
        .'<td width="40%">'
        .__('timeout').' :'
        .'</td>'
        ...
        if($systemConfig['scfg_by']=="outlet"&&!empty($periodIds)){
        $value.='<tr>'
        .'<td>'
        .__d('pos','period').' :'
        .'</td>'
        ...
        .'</td>'
        .'<td>'
        .(isset($values['display_loading_box_during_payment'])&&isset($yesNoOpts[$values['display_loading_box_during_payment']])?$yesNoOpts[$values['display_loading_box_during_payment']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;')
        .'</td>'
        .'</tr>'
        ...
        .'<tr>'
        .'<td>'
        .__d('pos','settlement_count_interval_to_print').' :'
        ...
        .'</td>';
        $value.='<td>'
        .'<table class="cell-table" width="99%">';
        ...
        .'<td style="border-right:none;width:25%">'.$tableStatusColor.'</td>'
        ...
        //	period
        $periodIds=explode(",",isset($values['period_ids'])?$values['period_ids']:"");
        if($systemConfig['scfg_by']=="outlet"){
        $value.='<tr>'
        .'<td>'
        .__d('pos','period').' :'
        .'</td>'
        ...
        .'</td>'
        .'<td>'
        .(isset($values['breakdown_at_check_settle'])&&isset($yesNoOpts[$values['breakdown_at_check_settle']])?$yesNoOpts[$values['breakdown_at_check_settle']]:__('unknown'))
        .'</td>'
        .'</tr>'
        .'</table>';
        break;
        ...
        break;
        case'cover_limit':
        if(is_numeric($systemConfig['scfg_value'])){
        $values['upper_bound']=$systemConfig['scfg_value'];
        $values['warning']=0;
        }else
        $values=json_decode($systemConfig['scfg_value'],true);
        $value='<table class="no-border" width="99%">'
        ...
        .'</td>'
        .'<td width="25%">'
        .($values['upper_bound']==0?__d('pos','no_limit'):$values['upper_bound'])
        .'</td>'
        .'</tr>'
        //	Cover Warning
        .'<tr>'
        ...
        .'</td>'
        .'<td width="25%">'
        .($values['warning']==0?__d('pos','no_limit'):$values['warning'])
        .'</td>'
        .'</tr>'
        .'</table>';
        break;
        ...
        .'</td>'
        .'<td>'
        .(isset($values['check_discount'])&&isset($yesNoOpts[$values['check_discount']])?$yesNoOpts[$values['check_discount']]:'&lt;&lt;&lt; '.__('unknown').' &gt;&gt;&gt;')
        .'</td>'
        .'</tr>'
        .'<tr>'
        .'<td width="50%">'
...
default:
        ...
        //	Remark
//					echo '<td>'.$systemConfig['scfg_remark'].'</td>';
        ...
        echo'<tr><td style="height:100px;text-align:center;vertical-align:middle;">'
        .__('no_record_found')
        .'</td></tr>';
        }
        ...
        //	Draw buttons
        echo'<div class="buttons-bar" style="padding-left: 0px;">';
        echo $this->element('button',array('id'=>'CancelButton','content'=>__("back"),'link'=>$this->Html->url('/admin'.$langPath.'pos/location_configs/listing'.$backUrlParamsStr)));
        echo'</div>';
